...
// Generata dall'enumerazione Contest_ID di ADIF 3.1.5.
constexpr Row kContests[] = {
    ...
    {"ARI-40-80", "ARI 40/80 Contest"},
    {"ARI-CAMPIONATO-HF", "ARI Campionato Italiano HF"},
    ...
    {"ARI-LOMBARDIA", "ARI Trofeo Lombardia"},
    {"ARI-MARCONI-HF", "ARI Marconi Memorial HF Contest"},
    {"ARI-MARCONI-VHF", "IARU Marconi Memorial VHF Contest"},
    {"ARI-SEZIONI", "ARI Trofeo Sezioni"},
    {"ARI-TROFEO-ADRIATICO", "ARI Trofeo Adriatico"},
    {"ARI-VHF-UHF-SHF", "ARI Campionato VHF-UHF-SHF"},
    ...
    {"EA-CNCW", "Concurso Nacional de Telegraf�a"},
    {"EA-DME", "Municipios Espa�oles"},
    ...
    {"EA-PSK63", "EA PSK63"},
    {"EA-RTTY ( import-only )", "Uni�n de Radioaficionados Espa�oles RTTY Contest"},
    {"EA-SMRE-CW", "Su Majestad El Rey de Espa�a - CW (2021 and earlier)"},
    {"EA-SMRE-SSB", "Su Majestad El Rey de Espa�a - SSB (2021 and earlier)"},
    {"EA-VHF-ATLANTIC", "Atl�ntico V-UHF"},
    ...
    {"EA-WW-RTTY", "Uni�n de Radioaficionados Espa�oles RTTY Contest"},
...
{
    static const QList<Contest> list = [] {
        QList<Contest> out;
        out.reserve(std::size(kContests));
        for (const Row& row : kContests)
            out << Contest{QString::fromLatin1(row.id), QString::fromLatin1(row.name)};
        return out;
    }();
    return list;
}
...
QList<Contest> search(const QString& text)
{
    ...
    if (needle.isEmpty())
        return all();
    QList<Contest> out;
    for (const Contest& c : all()) {
        if (c.id.contains(needle, Qt::CaseInsensitive) || c.name.contains(needle, Qt::CaseInsensitive))
            out << c;
    }
    return out;
}
...
QString nameFor(const QString& id)
{
    ...
    for (const Contest& c : all()) {
        if (c.id.compare(wanted, Qt::CaseInsensitive) == 0)
            return c.name;
    }
    return {};
}
